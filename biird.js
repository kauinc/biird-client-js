...
const baseURL = "http://api.biird.io";
const resourceValueURL = "http://api.biird.io/resourceValue/";
...
document.addEventListener("DOMContentLoaded", function () {
    var script = document.querySelectorAll(scriptSelector)[0];
    let baseDimensionJSON = script.getAttribute(dimensionsAttributeSelector);
    let baseDimension = {};
    if (baseDimensionJSON != undefined && baseDimensionJSON.length > 0) {
        baseDimension = JSON.parse(baseDimensionJSON);
    }
    ...
    for (i = 0; i < placeholders.length; i++) {
        ...
        let overrideDimensionJSON = placeholder.getAttribute(dimensionsAttributeSelector);
        let overrideDimension = {};
         if (overrideDimensionJSON != undefined && overrideDimensionJSON.length > 0) {
            overrideDimension = JSON.parse(overrideDimensionJSON)
        }
        let finalizedDimensions = {};
        finalizedDimensions = Object.assign(finalizedDimensions, baseDimension, overrideDimension);

        let url = encodeURI(resourceValueURL + biirdId + "?" + attributeParametersFromDimensions(finalizedDimensions));
    ...
    function attributeParametersFromDimensions(dimensions) {
        return Object.keys(dimensions).map(function (k) {
